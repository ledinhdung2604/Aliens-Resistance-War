...
void Player :: handleEvent (bool keys[]) {
    double currentSpeed = speed * speedIncrease * gravityFactor;
    // Di chuyen dua tren trang thai phim
    if (keys[SDL_SCANCODE_W] || keys[SDL_SCANCODE_UP]) {
        position.y -= static_cast <int> (currentSpeed);
    }

    if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_RIGHT]) {
        position.x -= static_cast <int> (currentSpeed);
    }
...
void Player :: update () {
    // Đảm bảo Player không ra khỏi màn hình
...
void Player :: applyBuff (buffType type, Uint32 buffLength) {
    switch (type) {
        case HEAL:
        health = min(health + 30, health); // Hồi 30 máu
        break;

        case SHIELD:
        shieldActivate = true; // kích hoạt shield
        break;

        case DAMAGE_BOOST:
        damageIncrease = 3.0; // tăng dame 3 lần
        break;

        case ATTACK_SPEED:
        attackSpeedUp = 2.0; // x2 tốc đánh
        break;

        case REFLECT:
        reflectActivate = true;  // phản đạn
        break;

        case UP_MAX_HP:
        maxHealth += 5;  // tăng 5 max health
        break;
        ...
        case SPEED_BOOST:
        speedIncrease = 2.0;  // x2 tốc độ di chuyển
        break;
    }
...
void Player :: setGravitySpeed (bool affect) {
    if (affect) {
        gravityFactor = 0.5; // giảm 2 lần khi bị lỗ đen ảnh hưởng
    }
